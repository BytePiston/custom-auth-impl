...
public class WebSecurityConfig {
	...
	private static final String[] WHITE_LIST_URLS = { "/api/v1/hello", "/api/v1/user/register",
			...
			"/api/v1/user/updatePassword*", "/api/v1/user/resendResetPasswordToken*", "/api/v1/user/changePassword"};
	 ...
	 * .permitAll().requestMatchers("/api/**").authenticated()) .oauth2Login(oauth2login
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.cors(AbstractHttpConfigurer::disable)
			...
			.authorizeHttpRequests(req -> req.shouldFilterAllDispatcherTypes(false)
